...
- 인증키는 .env 의 KRX_API= 값에서 읽음 (KRX_ID/KRX_PW 불필요).
...
def _get(path: str, params: dict) -> list:
    ...
    if not AUTH_KEY:
        raise KrxApiError(".env 에 KRX_API=<인증키> 가 없습니다.")
    url = f"{BASE}/{path}"
    try:
...
def stock_daily(bas_dd: str, market: str = "KOSPI") -> pd.DataFrame:
    """일별 전종목 매매정보. market: KOSPI(stk) / KOSDAQ(ksq) / KONEX(knx)."""
...
def index_daily(bas_dd: str, market: str = "KOSPI") -> pd.DataFrame:
    """일별 지수 시세. (idx 서비스 이용신청 필요) market: KOSPI/KOSDAQ/KRX."""
